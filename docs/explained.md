...
- **The assembly (the new headline):** best of all — we don't even need to copy that state. We
  ...
  **"Rome" as often as it normally answers anything** (52%, when its own accuracy is 53%). Swap
  ...
  Push 40× too hard and it babbles in loops — *"dollar dollar dollar…"* — yet even then, forced
  to pick among the possible answers, it still picks the right one **80% of the time**.
  Overdosing destroys fluency, not the information.)
...
**Want to see it with your own eyes?** [Assemble a thought](assembly.md) yourself and watch the
model speak the answer, or explore [the geometry](geometry.md) behind it. The full details, with every number and every
control, are in the [paper](assets/paper.pdf) and in [Evidence & controls](evidence.md).
